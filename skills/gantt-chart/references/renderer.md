...
- HTML builds a grouped product Gantt from the normalized data. It provides search, phase and owner filters; critical-only review; comparison and dependency toggles; zoom; keyboard row navigation; selected predecessor/successor highlighting; persistent details; visible-slice CSV; reset; and a print/PDF layout with active-filter context. There is no dragging, writeback, computed critical path or hierarchy collapse.
...
- The three download links contain the complete original snapshot irrespective of filters. The toolbar's **Export visible CSV** action intentionally exports the current review slice and names that scope in the filename. JSON is the authoritative editable model; CSV exports three task date layers and does not encode the dependency graph, presentation or calendar. Formula-leading CSV text receives an apostrophe. The SVG uses direct dates and supplied analysis labels; HTML/JSON retain full task evidence.
