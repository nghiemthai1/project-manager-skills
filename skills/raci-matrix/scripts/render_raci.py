...
def render_html(data, svg, csv_text):
    ...
    def search_text(row):
        return esc((json.dumps(row,ensure_ascii=False)+' '+ ' '.join(r['label'] for r in roles)).lower())
    table_rows=''.join(f'<tr data-row="{esc(row["id"])}" data-findings="{str(row["id"] in flag_ids).lower()}" data-search="{search_text(row)}"><th scope="row">{esc(row["id"])}<br>{esc(row["label"])}</th>{buttons(row)}</tr>' for row in rows)
    cards=''.join(f'<article data-row="{esc(row["id"])}" data-findings="{str(row["id"] in flag_ids).lower()}" data-search="{search_text(row)}"><h3>{esc(row["label"])}</h3><p>{esc(row["id"])}</p>'+''.join(f'<button class="card-cell" data-role="{esc(role["id"])}" data-detail="{esc(detail(row,role,row["cells"][role["id"]]))}">{esc(role["label"])} <b>{esc(row["cells"][role["id"]]["code"])}</b> · {esc(row["cells"][role["id"]]["state"])}</button>' for role in roles)+'</article>' for row in rows)
