...
Link worked material once from the Examples section and mark it optional. Keep long demonstrations and their data, rendered outputs, and downloads under `examples/` (use `examples/assets/` for supporting files). Reserve a package-level `assets/` folder for reusable production resources. Application and references should teach the method using the user’s project data without requiring repeated visits to fictional scenarios.
