...
[Visual evaluation record](../evals/visual/README.md) distinguishes independent fictional executions, corrected outputs, browser checks and structural checks. Local validation passes all 45 unit tests, including isolated renderer execution and package payload comparison. This does not certify screen-reader behavior or field adoption.

The included Gantt renderer is a read-only, date-only snapshot viewer. It preserves typed relationships and calendars but does not implement vendor adapters, resource leveling, intraday scheduling, editing, live writes or critical-path calculation. It can display explicitly supplied and method-labeled criticality/float analysis. Its product view groups phases in one sticky schedule grid, filters the visible review slice, redraws dependency links, highlights a selected chain, and exposes that scope in both the UI and print context. Permanent SVG/JSON/CSV downloads preserve the complete snapshot; the explicitly labeled visible-slice CSV action exports the current filter result. The wider guide explains when a different renderer or schedule engine is needed. The RACI helper reports evidence/assignment gaps without approving roles.
