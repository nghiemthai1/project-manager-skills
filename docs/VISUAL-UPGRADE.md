...
A review artifact can be a standalone HTML file; it does not require a web app, server, tracker integration or live writeback. The RACI and Dependency Map artifacts now support explicitly local draft editors with validation, immutable source data, before/after history, undo, source restore and separate draft exports. RACI recalculates its audit views; Dependency Map recalculates its graph, DSM, register and local metrics while retaining cycles for coordination review. Neither makes a multi-user or live-system claim. Schedule editing remains a separate mode with calendar/dependency validation, undo and conflict handling. A renderer does not become a scheduling engine by drawing linked bars. A colored or edited RACI cell does not confirm an assignment.
...
- Three original standalone renderers: Gantt, RACI and Dependency Map. Each fictional software/migration case includes offline HTML, SVG, exact source JSON and spreadsheet-safe CSV. Gantt includes its full source-ingestion, data-contract, interaction, performance, accessibility and mobile design references. RACI adds matrix, row-audit and role-profile views with declared thresholds, evidence detail, separate assignment/confirmation semantics and browser-local draft editing with validation/history/undo. Dependency Map adds a directed graph, DSM, evidence register, local-margin controls, an explicit CPM boundary and browser-local agreement editing with cycle warnings, validation, history, undo and source/draft export separation.
...
- Seven additional rendered software companions cover budget, benefits, capacity, WBS, stakeholder mapping, risk evidence and release readiness. Their source tables, assumptions and editable JSON accompany the SVGs. Dependency Map has moved from this static-companion set to a dedicated software/migration renderer.
...
The included Gantt renderer is a read-only, date-only snapshot viewer. It preserves typed relationships and calendars but does not implement vendor adapters, resource leveling, intraday scheduling, editing, live writes or critical-path calculation. It can display explicitly supplied and method-labeled criticality/float analysis. Its product view groups phases in one sticky schedule grid, filters the visible review slice, redraws dependency links, highlights a selected chain, and exposes that scope in both the UI and print context. Permanent SVG/JSON/CSV downloads preserve the complete snapshot; the explicitly labeled visible-slice CSV action exports the current filter result. The Dependency Map renderer calculates only comparable calendar-day local margins, preserves unknowns and acceptance states, and does not infer commitments, critical paths or project delay; its local editor never writes to source files and marks retained analysis for revalidation. The RACI helper audits rows and role patterns without approving roles, inferring authority or treating assignment counts as workload.
