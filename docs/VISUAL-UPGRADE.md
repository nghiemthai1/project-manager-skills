...
- Three original standalone renderers: Gantt, RACI and Dependency Map. Each fictional software/migration case includes offline HTML, SVG, exact source JSON and spreadsheet-safe CSV. Gantt includes its full source-ingestion, data-contract, interaction, performance, accessibility and mobile design references. RACI adds matrix, row-audit and role-profile views with declared thresholds, evidence detail and separate assignment/confirmation semantics. Dependency Map adds a directed graph, DSM, evidence register, local-margin controls and an explicit CPM boundary.
