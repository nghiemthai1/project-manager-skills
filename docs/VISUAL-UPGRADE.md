...
A read-only artifact can be a standalone HTML file; it does not require a web app, server, tracker integration or live writeback. The RACI artifact now supports an explicitly local draft editor with cell validation, immutable source data, before/after history, undo, source restore and separate draft exports. It recalculates audit views after edits while making no multi-user or live-system claim. Schedule editing remains a separate mode with calendar/dependency validation, undo and conflict handling. A renderer does not become a scheduling engine by drawing linked bars. A colored or edited RACI cell does not confirm an assignment.
