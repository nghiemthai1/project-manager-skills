...
Run `python scripts/validate.py --build-catalog`, both authoring check scripts, and `python -m unittest discover -s tests -v`. Automated routing checks verify metadata and curated case availability; this revision does not claim another full model-routing run across all 45 packages.
